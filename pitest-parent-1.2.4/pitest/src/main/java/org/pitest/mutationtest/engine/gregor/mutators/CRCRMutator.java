...
public class CRCRMutator implements MethodMutatorFactory {
    ...
    private final class CRCRMutatorVisitor extends MethodVisitor {
        ...
        CRCRMutatorVisitor(final MutationContext context, final MethodVisitor delegateVisitor) {
        ...
        private void mutate(final Double constant) {
            ...
            double temp = constant.doubleValue();
            if (temp <= Short.MIN_VALUE || temp >= Short.MAX_VALUE) {             
                replacement = -constant;
            } else if (temp <= Byte.MIN_VALUE) {          
                replacement = Double.valueOf(0);
            } else if (temp >= Byte.MAX_VALUE) {
                replacement = Double.valueOf(1);
            } else {
                replacement = constant + 1D;
            }
        ...
        private void mutate(final Float constant) {
            ...
            final Float replacement;
            
            double temp = constant.doubleValue();
            if (temp <= Short.MIN_VALUE || temp >= Short.MAX_VALUE) {             
                replacement = -constant;
            } else if (temp <= Byte.MIN_VALUE) {          
                replacement = Float.valueOf(0);
            } else if (temp >= Byte.MAX_VALUE) {
                replacement = Float.valueOf(1);
            } else {
                replacement = constant + 1F;
            }
        ...
        private void mutate(final Long constant) {
            ...
            long temp = constant.longValue();
            if (temp <= Short.MIN_VALUE || temp >= Short.MAX_VALUE) {             
                replacement = -constant;
            } else if (temp <= Byte.MIN_VALUE) {          
                replacement = Long.valueOf(0);
            } else if (temp >= Byte.MAX_VALUE) {
                replacement = Long.valueOf(1);
            } else {
                replacement = constant + 1L;
            }
            if (shouldMutate(constant, replacement)) {
                translateToByteCode(replacement);
            } else {
        ...
        private <T extends Number> boolean shouldMutate(final T constant, final T replacement) {
            final MutationIdentifier mutationId = this.context.registerMutation(CRCRMutator.this,
                    "Substituted " + constant + " with " + replacement);
